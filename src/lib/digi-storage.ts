...
const getRequiredEnv = (names: string[]) => {
  ...
  if (!value) {
    throw new Error(`Missing Digi Storage env var. Set one of: ${names.join(', ')}.`)
  }
...
const getConfig = (): DigiConfig => {
  const baseUrl = (process.env.DIGI_STORAGE_BASE_URL || defaultBaseUrl).replace(/\/+$/g, '')
  const apiUrl = (process.env.DIGI_STORAGE_API_URL || `${baseUrl}/api/v2.1`).replace(/\/+$/g, '')
  const basePath =
    process.env.DIGI_STORAGE_BASE_PATH || process.env.ASSET_FTP_BASE_PATH || defaultBasePath

  return {
    apiUrl,
    basePath: trimSlashes(basePath),
    baseUrl,
    email: getRequiredEnv(['DIGI_STORAGE_EMAIL', 'DIGI_EMAIL', 'ASSET_FTP_USERNAME']),
    mountId: process.env.DIGI_STORAGE_MOUNT_ID?.trim() || undefined,
    password: getRequiredEnv(['DIGI_STORAGE_PASSWORD', 'DIGI_PASSWORD', 'ASSET_FTP_PASSWORD']),
  }
}
...
export const getDigiImageMaxBytes = () => {
  ...
  return Number.isFinite(configured) && configured > 0 ? configured : defaultImageMaxBytes
}
...
const authenticate = async (): Promise<DigiAuthState> => {
  ...
  const mounts = Array.isArray(mountsResponse) ? mountsResponse : mountsResponse.mounts || []
...
const withAuthRetry = async <T>(operation: (auth: DigiAuthState) => Promise<T>) => {
  try {
...
const createFolder = async (auth: DigiAuthState, parentPath: string, name: string) => {
  const config = getConfig()
  const response = await fetch(
...
const getRemotePath = (objectPath: string) => {
  const config = getConfig()
  const combined = [config.basePath, trimSlashes(objectPath)].filter(Boolean).join('/')

  return `/${combined}`
}

export const prepareDigiStorageUpload = async ({ objectPath }: DigiPreparedUploadInput) =>
...
export const getDigiStorageDownloadLink = async (storagePath: string) =>
  withAuthRetry(async (auth) => {
    const config = getConfig()
    const normalizedPath = storagePath.startsWith('/') ? storagePath : `/${trimSlashes(storagePath)}`
