...
export const sendLeadNotification = async (lead: LeadNotificationInput) => {
  ...
  const from =
    process.env.CONTACT_FROM_EMAIL || "VD BARRISOL <onboarding@resend.dev>";
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      html: `
        ...
        <p>${escapeHtml(lead.specification || "")}</p>
    ...
    headers: {
      ...
      ...(lead.requestKey ? { "Idempotency-Key": lead.requestKey } : {}),
    },
    method: "POST",
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Lead email failed (${response.status})${details ? `: ${details.slice(0, 300)}` : ""}`,
    );
  }
