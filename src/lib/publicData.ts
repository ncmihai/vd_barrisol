...
const imageFromAsset = (asset: unknown, fallback: PublicImage): PublicImage => {
  ...
  const variants = (item.variants && typeof item.variants === 'object' ? item.variants : {}) as AnyRecord
  const preferredVariant =
    item.preferredVariant && item.preferredVariant !== 'auto' ? item.preferredVariant : '2k'
  const variant =
    preferredVariant === '1080'
      ...
      : preferredVariant === 'original'
        ...
        : preferredVariant === 'thumbnail'
          ...
          : ((variants.twoK || variants.hd1080 || variants.original || variants.thumbnail) as
              | AnyRecord
              | undefined)
  ...
  return {
    ...
    height: typeof variant.height === 'number' ? variant.height : fallback.height,
...
export const getPublicSiteData = async (locale: Locale): Promise<PublicSiteData> => {
  ...
  try {
    ...
    return {
      ...
      settings,
      testimonials: mappedTestimonials.length > 0 ? mappedTestimonials : fallback.testimonials,
    }
  } catch (error) {
    console.warn('[public-data] using fallback content', error instanceof Error ? error.message : error)

    return fallback
  }
}

const mapSiteSettings = (doc: AnyRecord, fallback: PublicSiteSettings): PublicSiteSettings => {
  ...
  return {
    ...
    whatsappHref: whatsappHref(doc.whatsappNumber),
    whatsappNumber: nonEmptyString(doc.whatsappNumber, fallback.whatsappNumber),
  }
}
...
const mapHomePage = (doc: AnyRecord, fallback: PublicHomePage): PublicHomePage => {
  ...
  return {
    ...
    calculatorHeadline: nonEmptyString(calculator.headline, fallback.calculatorHeadline),
        ...
        ? slides.map((rawSlide, index) => {
            ...
            return {
              caption: nonEmptyString(
                slide.caption,
                fallback.heroSlides[index % fallback.heroSlides.length]?.caption || '',
              ),
              image: imageFromAsset(
                slide.image,
                fallback.heroSlides[index % fallback.heroSlides.length]?.image ||
                  fallback.heroSlides[0].image,
              ),
            }
          })
        : fallback.heroSlides,
...
const mapGalleryPage = (doc: AnyRecord, fallback: PublicGalleryPage): PublicGalleryPage => {
  const seo = asRecord(doc.seo)
...
const mapAboutPage = (doc: AnyRecord, fallback: PublicAboutPage): PublicAboutPage => {
  const seo = asRecord(doc.seo)
...
const mapProject = (doc: AnyRecord, fallback: PublicProject): PublicProject => ({
...
const mapTestimonial = (doc: AnyRecord, fallback: PublicTestimonial): PublicTestimonial => ({
  city: nonEmptyString(doc.city, fallback.city || ''),
  clientName: nonEmptyString(doc.clientName, fallback.clientName),
  headline: nonEmptyString(doc.headline, fallback.headline || ''),
  image: doc.image ? imageFromAsset(doc.image, fallback.image || fallbackImage) : undefined,
